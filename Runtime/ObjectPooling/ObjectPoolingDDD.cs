...
public class ObjectPoolingDDD : MonoBehaviour
{
    #region 静态部分
    public static List<ObjectPoolingDDD> instances;
    #endregion
    ...
    {
        if(instances == null)
            instances = new List<ObjectPoolingDDD>();
    ...
    public void Deactivation(GameObject g)
    {
        ...
        if(resetProperty)
        {
            foreach (var item in prefab.GetComponents<Component>())
            {
                if(rules)
                {
                    if(rules.GetRules() != null && System.Array.IndexOf(rules.GetRules(),item.name) >= 0)
                    {
                        continue;
                    }
                }
